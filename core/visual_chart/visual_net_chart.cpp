...
#include <stdint.h>
#include <errno.h>
Visual_NET_Chart::Visual_NET_Chart(QWidget *parent)
    :Visual_Basic_Chart(parent)
{
    ...
    connect(updateTimer, &QTimer::timeout, this, &Visual_NET_Chart::timeout_slot);
    show();
    timeout_slot();
    updateTimer->start();
}
void Visual_NET_Chart::timeout_slot()
    {
        updateTimer->setInterval(update_time);
        get_net_data();
        if (last_Tra == 0)
...
Visual_NET_Chart::~Visual_NET_Chart()
{
    if (visual_net_chart_list)
    {
        visual_net_chart_list->removeOne(this);
    }
}
void Visual_NET_Chart::get_net_data()
{
...
void Visual_NET_Chart::save(QSettings *settings)
{
...
void Visual_NET_Chart::load(QSettings *settings)
{
...
void Visual_NET_Chart::set_icon(QString checked_icon_path)
{
...
void Visual_NET_Chart::contextMenuEvent(QContextMenuEvent *event)
{
