...
#include <stdint.h>
#include <errno.h>
Visual_DISK_Chart::Visual_DISK_Chart(QWidget *parent)
    :Visual_Basic_Chart(parent)
{
    ...
    connect(updateTimer, &QTimer::timeout, this, &Visual_DISK_Chart::timeout_slot);
    show();
    timeout_slot();
    updateTimer->start();
}
void Visual_DISK_Chart::timeout_slot()
{
    updateTimer->setInterval(update_time);
    get_disk_data();
    if (last_Tra == 0)
...
Visual_DISK_Chart::~Visual_DISK_Chart()
{
    if (visual_disk_chart_list)
    {
        visual_disk_chart_list->removeOne(this);
    }
}
void Visual_DISK_Chart::get_disk_data()
{
    ...
    {
        if (strstr(line, "loop") || strstr(line, "dm-") || strstr(line, "ram") || strstr(line, "sr"))
        {
            continue;
        }
        if (!(strstr(line, "sd") || strstr(line, "hd") || strstr(line, "nvme") || strstr(line, "vd")))
        {
            continue;
        }
        unsigned long long index = 1,Rec_data = 0, Tra_data = 0;
        sscanf(line, "%*d %llu %*s %*u %*u %llu %*u %*u %*u %llu %*u", &index, &Rec_data, &Tra_data);
        //if (index != 0) return;//都怪你,泄漏了
        if (index != 0) continue;
        new_Tra += Tra_data;//读取
        new_Rec += Rec_data;//写入
    }
...
void Visual_DISK_Chart::save(QSettings *settings)
{
...
void Visual_DISK_Chart::load(QSettings *settings)
{
    ...
    line1_color = QColor::fromRgba(settings->value("line1_color", QColor(106, 215, 135, 255).rgba()).toUInt());
    line2_color = QColor::fromRgba(settings->value("line2_color", QColor(143, 136, 225, 255).rgba()).toUInt());
...
void Visual_DISK_Chart::set_icon(QString checked_icon_path)
{
...
void Visual_DISK_Chart::contextMenuEvent(QContextMenuEvent *event)
{
