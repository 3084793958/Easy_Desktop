...
#include <stdint.h>
#include <errno.h>
Visual_CPU_Chart::Visual_CPU_Chart(QWidget *parent)
    :Visual_Basic_Chart(parent)//不要相信QT的预编译
{
    ...
    connect(updateTimer, &QTimer::timeout, this, &Visual_CPU_Chart::timeout_slot);
    show();
    timeout_slot();
    updateTimer->start();
}
void Visual_CPU_Chart::timeout_slot()
{
    updateTimer->setInterval(update_time);
    get_cpu_data();
    ...
    if (channel == -1)
    {
        ...
        else if (send_cpu_data_list.count() - 2 > series_list.count())
        {
            while (send_cpu_data_list.count() - 2 > series_list.count())
            {
                Visual_QLineSeries *new_series = new Visual_QLineSeries(this);
...
Visual_CPU_Chart::~Visual_CPU_Chart()
{
    if (visual_cpu_chart_list)
    {
        visual_cpu_chart_list->removeOne(this);
    }
}
void Visual_CPU_Chart::save(QSettings *settings)
{
...
void Visual_CPU_Chart::load(QSettings *settings)
{
...
void Visual_CPU_Chart::get_cpu_data()
{
    each_cpu_data.clear();
...
void Visual_CPU_Chart::contextMenuEvent(QContextMenuEvent *event)
{
